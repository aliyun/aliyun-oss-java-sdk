...
import static com.aliyun.oss.integrationtests.TestConstants.SECURITY_TOKEN_NOT_SUPPORTED_ERR;
...
public class SecurityTokenTest {
	...
	public void testBucketOperationsWithToken() {
		...
		try {
			...
			Assert.fail("List buckets should not be successful");
		} catch (OSSException e) {
			Assert.assertEquals(OSSErrorCode.SECURITY_TOKEN_NOT_SUPPORTED, e.getErrorCode());
			Assert.assertTrue(e.getMessage().startsWith(SECURITY_TOKEN_NOT_SUPPORTED_ERR));
		} finally {
			actions.clear();
			resources.clear();
