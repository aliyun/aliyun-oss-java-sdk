...
public class UploadPartTest extends TestBase {
	...
	public void testUnormalAbortMultipartUpload() {
		...
		// unormal-abort-multipart-upload-existing-bucket
		...
		try {
			...
			AbortMultipartUploadRequest abortMultipartUploadRequest = new AbortMultipartUploadRequest(existingBucket,
					key, uploadId);
			ossClient.abortMultipartUpload(abortMultipartUploadRequest);
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		...
		try {
			...
			for (int i = 0; i < multipartUploadCount; i++) {
				AbortMultipartUploadRequest abortMultipartUploadRequest = new AbortMultipartUploadRequest(bucketName,
						existingKeys.get(i), uploadIds.get(i));
				ossClient.abortMultipartUpload(abortMultipartUploadRequest);
			}
	...
	public void testNormalCompleteMultipartUpload() {
		...
		try {
			...
			completeMultipartUploadRequest = new CompleteMultipartUploadRequest(bucketName, key, uploadId,
					discontinuousPartETags);
