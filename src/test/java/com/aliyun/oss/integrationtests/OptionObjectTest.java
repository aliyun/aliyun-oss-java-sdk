...
public class OptionObjectTest extends TestBase {
	...
	public void testOptionObject() {
		...
		/*
		 * // Enable bucket cors SetBucketCORSRequest setBucketCORSRequest = new
		 * SetBucketCORSRequest(bucketName);
		 * 
		 * CORSRule r0 = new CORSRule(); r0.addAllowdOrigin("http://www.example.com");
		 * r0.addAllowedMethod("PUT"); r0.addExposeHeader("x-oss-test");
		 * setBucketCORSRequest.addCorsRule(r0);
		 * 
		 * secondClient.setBucketCORS(setBucketCORSRequest);
		 * 
		 * // Now options object will be successful ResponseMessage response =
		 * secondClient.optionsObject(optionsRequest); Map<String, String>
		 * responseHeaders = response.getHeaders(); Assert.assertEquals(origin,
		 * responseHeaders.get(OSSHeaders.ORIGIN)); Assert.assertEquals(requestHeaders,
		 * responseHeaders.get(OSSHeaders.ACCESS_CONTROL_REQUEST_HEADER));
		 * Assert.assertEquals(HttpMethod.PUT,
		 * responseHeaders.get(OSSHeaders.ACCESS_CONTROL_ALLOW_METHODS));
		 */
	}
}
