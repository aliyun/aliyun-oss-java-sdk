...
public class BucketReplicationTest extends TestBase {
	...
	public static void beforeClass() {
		if (replicationClient == null) {
			replicationClient = new OSSClientBuilder().build(OSS_TEST_REPLICATION_ENDPOINT,
					OSS_TEST_REPLICATION_ACCESS_KEY_ID, OSS_TEST_REPLICATION_ACCESS_KEY_SECRET);
	...
	public void testNormalAddBucketReplicationWithDefaultRuleID() throws ParseException {
		...
		try {
			...
			ossClient
					.deleteBucketReplication(new DeleteBucketReplicationRequest(bucketName, r0.getReplicationRuleID()));
	...
	public void testNormalDeleteBucketReplicationWithRuleID() throws ParseException {
		...
		try {
			...
			ossClient.deleteBucketReplication(
					new DeleteBucketReplicationRequest(bucketName).withReplicationRuleID(repRuleID));
	...
	public void testNormalGetBucketReplicationProgressWithDisableHistory() throws ParseException {
		...
		try {
			...
			BucketReplicationProgress process = ossClient.getBucketReplicationProgress(
					new GetBucketReplicationProgressRequest(bucketName).withReplicationRuleID(repRuleID));
			Assert.assertEquals(process.getReplicationRuleID(), repRuleID);
			Assert.assertEquals(process.getTargetBucketName(), targetBucketName);
			Assert.assertEquals(process.getTargetBucketLocation(), targetBucketLoc);
			Assert.assertEquals(process.getReplicationStatus(), ReplicationStatus.Starting);
			...
			Assert.assertEquals(process.getNewObjectProgress(), null);
			// Assert.assertEquals(diffSecond(process.getNewObjectProgress(), now) < 5,
			// true);
		...
		try {
			...
			try {
				...
				Assert.assertEquals(e.getMessage()
						.startsWith("The target bucket you specified does not locate in the target location"), true);
			}
	...
	public void testUnormalGetBucketReplication() throws ParseException {
		...
		try {
			...
			try {
				...
				Assert.assertEquals(
						e.getMessage().startsWith("The bucket you specified does not have replication configuration"),
						true);
			}
	...
	public void testUnormalGetBucketReplicationProgress() throws ParseException {
		...
		try {
			...
			try {
				...
				Assert.assertEquals(e.getMessage().startsWith("The BucketReplicationRule you specified does not exist"),
						true);
			}
