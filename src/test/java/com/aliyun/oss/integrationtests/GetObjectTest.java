...
public class GetObjectTest extends TestBase {
		...
		try {
			...
			for (int i = 0; i < threadCount; i++) {
				...
				Runnable r = new Runnable() {
					...
					public void run() {
						try {
							ossClient.putObject(bucketName, buildObjectKey(keyPrefix, seqNum), new File(filePath));
		...
		try {
			...
			for (int i = 0; i < threadCount; i++) {
				...
				Runnable r = new Runnable() {
					...
					public void run() {
						try {
							ossClient.putObject(bucketName, buildObjectKey(keyPrefix, seqNum), randomFile);
							completedCounter.incrementAndGet();
						} catch (Exception ex) {
							Assert.fail(ex.getMessage());
						}
	...
	public void testGetObjectByRange() {
		...
		try {
			...
			// Invalid range exceeding object's max length, ignore it and just get entire
			// object
			getObjectRequest.setRange(0, inputStreamLength * 2 - 1);
			o = ossClient.getObject(getObjectRequest);
			Assert.assertEquals(inputStreamLength, o.getObjectMetadata().getContentLength());
