...
public class SwitchCredentialsAndEndpointTest extends TestBase {
	...
	/*
	 * Indicate whether credentials switching starts prior to credentials
	 * verification
	 */
	...
	public void testSwitchValidCredentialsAndEndpoint() {
		...
		try {
			...
			// After switching both credentials and endpoint, the default OSSClient is the
			// same
	...
	public void testSwitchCredentialsSynchronously() throws Exception {
		...
		Thread switchThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				do {
					Credentials secondCreds = new DefaultCredentials(OSS_TEST_ACCESS_KEY_ID,
					 ...
					 * TODO: The two synchronized clauses below should be combined as atomic
					 * operation.
					 */
					synchronized (verifySynchronizer) {
	...
	public void testSwitchEndpointSynchronously() throws Exception {
		...
		Thread verifyThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				do {
					...
					/*
					 * Since the default OSSClient is the same as the second OSSClient, let's do a
					 * simple verification.
					 */
		...
		Thread switchThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				do {
					/*
					 * Switch both credentials and endpoint, now the default OSSClient is the same
					 * as the second OSSClient actually.
					 */
					Credentials secondCreds = new DefaultCredentials(OSS_TEST_ACCESS_KEY_ID,
							OSS_TEST_ACCESS_KEY_SECRET);
