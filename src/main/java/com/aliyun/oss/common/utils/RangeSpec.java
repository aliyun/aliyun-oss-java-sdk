...
public class RangeSpec {
	...
	private static final Log log = LogFactory.getLog(RangeSpec.class);
	...
	public static RangeSpec parse(long[] range) {
		if (range == null || range.length != 2) {
			if (log.isWarnEnabled()) {
				log.warn("Invalid range value " + range + ", ignore it and just get entire object");
			}
		}
		...
		if (start < 0 && end < 0 ||
			...
			if (log.isWarnEnabled()) {
				log.warn("Invalid range value [" + start + ", " + end + "], ignore it and just get entire object");
			}
		}
