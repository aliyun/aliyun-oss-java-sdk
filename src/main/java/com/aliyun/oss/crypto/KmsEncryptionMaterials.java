...
public class KmsEncryptionMaterials implements EncryptionMaterials {
    ...
    private final LinkedHashMap<String, Map<String, String>> regionDescMaterials = 
    ...
    public KmsEncryptionMaterials(String region, String cmk) {
        ...
        regionDescMaterials.put(region, desc);
    }
    ...
    /**
     * Sets the credentials provider and construct an kms Client.
     */
    public void setKmsCredentialsProvider(CredentialsProvider credentialsProvider) {
        ...
        accessKeySecret = credentialsProvider.getCredentials().getSecretAccessKey();
        stsToken = credentialsProvider.getCredentials().getSecurityToken();
        IClientProfile profile = DefaultProfile.getProfile(region, accessKeyId, accessKeySecret);
        this.kmsClient = new DefaultAcsClient(profile);
    }

    /**
     * Create a new kms client by inner credentials info.
     */
    private DefaultAcsClient newKmsClient(String region) {
        IClientProfile profile = DefaultProfile.getProfile(region, accessKeyId, accessKeySecret);
    ...
    private EncryptResponse encryptPlainText(String keyId, String plainText) throws ClientException {
        ...
        if (stsToken != null) {
            encReq.setSecurityToken(stsToken);
        }
     ...
     */
    private DecryptResponse decryptCipherBlob(DefaultAcsClient kmsClient, String cipherBlob) throws ClientException {
        ...
        if (stsToken != null) {
            decReq.setSecurityToken(stsToken);
        }
    ...
    /**
     * Add other cmk's region and its descrption.
     * 
     * @param region
     *            region.
     * @param description
     ...
     */
    public synchronized void addRegionDescMaterial(String region, Map<String, String> description) {
        assertParameterNotNull(region, "region");
        if (description != null) {
            regionDescMaterials.put(region, new HashMap<String, String>(description));
        } else {
            regionDescMaterials.put(region, new HashMap<String, String>());
        }
    }
     ...
     */
    private String findRegionByDescription(Map<String, String> desc) {
        if (desc == null) {
            return null;
        }
        for (Map.Entry<String, Map<String, String>> entry : regionDescMaterials.entrySet()) {
    ...
    public void decryptCEK(ContentCryptoMaterialRW contentMaterialRW) {
        ...
        try {
            String region = findRegionByDescription(contentMaterialRW.getMaterialsDescription());
            if (region == null) {
                Entry<String, Map<String, String>> entry = getTailByReflection(regionDescMaterials);
                region = entry.getKey();
            }

            DefaultAcsClient kmsClient = newKmsClient(region);
            DecryptResponse decryptIvResp = decryptCipherBlob(kmsClient,
                    BinaryUtil.toBase64String(contentMaterialRW.getEncryptedIV()));
            byte[] iv = BinaryUtil.fromBase64String(decryptIvResp.getPlaintext());
            DecryptResponse decryptCEKResp = decryptCipherBlob(kmsClient,
