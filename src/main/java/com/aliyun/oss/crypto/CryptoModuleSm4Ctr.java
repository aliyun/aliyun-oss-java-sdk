...
/**
 * The crypto module specified to SM4_CTR crypto shecme.
 * that it will encrypt content with SM4_CTR algothrim.
 */
class CryptoModuleSm4Ctr extends CryptoModuleBase {
    CryptoModuleSm4Ctr(OSSDirect OSS,
    ...
    byte[] generateIV() {
        ...
        if (cryptoConfig.getContentCryptoMode().equals(ContentCryptoMode.SM4_CTR_MODE)) {
            for (int i = 8; i < 12; i++) {
                iv[i] = 0;
            }
        }
